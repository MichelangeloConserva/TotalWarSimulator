...
class Unit:
  ...
  @property
  def max_width(self):
    return self.calc_width(self.n // 3)

  @property
  def min_width(self):
    return self.calc_width(self.n // 10)

  @property
  def width(self):
    return self.calc_width(len(self.ranks[0]))

  @property
  def pos(self):
    return sum(self.soldiers_pos) / len(self.soldiers)

  @property
  def soldiers_pos(self):
    return [s.body.position for s in self.soldiers]
  ...
  """
    ...
    here_formation, _ = self.formation.get_formation(
      np.array(list(start_pos)), angle, n_ranks, self.n, size, dist
    )
    # The formation we actually want to go to
    dest_formation, ranks_ind = self.formation.get_formation(
      np.array(list(final_pos)), final_angle, n_ranks, self.n, size, dist
    )
  ...
  def update(self, dt):
    ...
    for i, s in enumerate(self.soldiers):
      ...
      if s.target_position is None:
        continue
      if len(ds) == 0:
        d = (s.body.position - s.target_position).length
      else:
        d = ds[i]
      ...
      if len(s.enemy_melee_range):
        speed = s.base_speed * self.ATTACKING_MULTIPLIERS
      else:
        speed = s.base_speed
      ...
      s.move(
        ...
        self.ATTACKING_MULTIPLIERS,
        self.LATERAL_NOISE_MULTIPLIER,
      )
