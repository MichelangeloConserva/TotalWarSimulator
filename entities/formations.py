...
class BaseFormation:
  ...
  def remove_springs(self):
    for s in self.intra_springs:
      if s in self.space._constraints: self.space.remove(s)
    self.intra_springs = set()
  ...
  def right_nn_spring(self, s, n_r):
    ...
    if not n_r.front_nn is None:
      ...
      joint = pymunk.PinJoint(holder, s.holder)
      joint.distance = (self.unit.rest_length_intra**2+self.unit.rest_length_intra**2)**0.5
      joint.collide_bodies = False
      self.space.add(joint)
      self.intra_springs.add(joint)
  ...
  def add_intra_springs_and_set_soldier_dest(self):
    ...
    for s in self.unit.soldiers: s.reset_nn()
    ...
    for i in range(len(ranks)-1):
      for j in range(len(ranks[i])):
        s = ranks[i][j]
        s.target_position = Vec2d(self.unit.order[k].tolist()); k += 1
  ...
  def get_formation(self, *args, **kwargs):      raise NotImplementedError("get_formation")
  def execute_formation(self, *args, **kwargs):  raise NotImplementedError("execute_formation")
  def substitute_dead(self, *args, **kwargs):    raise NotImplementedError("substitute_dead")
