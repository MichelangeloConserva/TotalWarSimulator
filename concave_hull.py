...
def SortByAngle(kNearestPoints, currentPoint, prevPoint):
  ...
  for NearestPoint in kNearestPoints:
    # calculate the angle
    angle = np.arctan2(
      NearestPoint[1] - currentPoint[1], NearestPoint[0] - currentPoint[0]
    ) - np.arctan2(prevPoint[1] - currentPoint[1], prevPoint[0] - currentPoint[0])
...
def concaveHull(dataset, k):
  ...
  while (
    not np.array_equal(firstpoint, currentPoint) or (step == 2)
  ) and points.size > 0:
